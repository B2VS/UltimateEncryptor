...
int main()
{
    ...
    while (inputFile)
    {
        ...
        stringToInts(input, inputString);
        for (int i = 0; i < input.size(); ++i)
        {
            if (input[i] < 0 || input[i] > 52) goto breakout;
            int index_sum = 0;
            for (int i = 0; i < key.size(); ++i)
                index_sum = (index_sum + i * key[i]) % 97;
            int pos = hiddenKeys[(key.size() * 101 + index_sum) % 53][index_sum % 11];
            int keyRow = abs(pos * 101 * cos(key[pos % key.size()]) + pos % 2) % 53;
            output.push_back((input[i] + hiddenKeys[keyRow][(2*index_sum + 7*i + 3*key[key.size() - 1 - i % key.size()]) % 11]) % 53);
        }
        intsToString(output, outputString);
