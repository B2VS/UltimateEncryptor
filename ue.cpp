...
void preprocessing(vector <int> &input)
{
    for (int i = input.size() - 2; i >= 0; --i)
        input[i] = (input[i] + input[i + 1]) % 53;
}
...
int main()
{
    ...
    ifstream inputFile("Einput.txt");
    ofstream outputFile("Eoutput.txt");
    ...
    while (inputFile)
    {
        ...
        if (inputString.size() > 10000 || inputString.size() == 0 || inputFile.bad() || inputFile.fail())
            return 0;
        stringToInts(plainText, inputString);
        preprocessing(plainText);
        for (int i = 0; i < plainText.size(); ++i)
        {
            if (plainText[i] < 0 || plainText[i] > 52) return 0;
            int pos = hiddenKeys[(key.size() * 101 + plainText.size() * 97) % 53][index_sum % 11];
            ...
            cipherText.push_back((plainText[i] + hiddenKeys[keyRow][keyCol]) % 53);
            offset = (2 * offset + plainText[i] * 7) % 97;
        }
        intsToString(cipherText, outputString);
